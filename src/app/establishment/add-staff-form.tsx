...
export function AddStaffForm({ setOpen }: { setOpen: (open: boolean) => void }) {
  ...
  function onSubmit(data: StaffFormValues) {
    ...
    startTransition(() => {
      ...
      setDoc(newEmployeeRef, newEmployeeData)
        .then(() => {
          if (data.hasSystemAccess && normalizedEmail) {
          ...
          toast({
            ...
            description: data.hasSystemAccess 
              ? `${data.name} added. System account provisioned with default password: 123456` 
