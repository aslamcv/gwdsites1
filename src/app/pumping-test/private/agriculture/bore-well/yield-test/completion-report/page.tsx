...
function ReportContent() {
  ...
  useEffect(() => {
    if (report) {
      document.title = `YT-Completion-BW-${report.fileNo || report.id.slice(0,6)}.pdf`;
    }
  }, [report]);
  ...
  const tableData = [
    ...
    { label: 'Type of well', value: report?.typeOfWell || 'Bore Well' },
    ...
    { label: 'Height of Measuring Point (agl)', value: report?.measuringPointHt },
    ...
    <div className="min-h-screen bg-slate-100 py-4 px-4 pt-12 print:bg-white print:p-0 font-sans text-black">
      ...
      <div className="bg-white mx-auto w-full max-w-[210mm] min-h-[297mm] shadow-xl print:shadow-none p-[12mm] flex flex-col text-[11px] leading-tight border border-slate-200 print:border-none overflow-hidden relative">
        ...
        <div className="mt-6 pt-1.5 border-t border-slate-200 text-[7px] text-slate-400 flex justify-between uppercase tracking-widest font-sans font-bold">
          ...
          <span>OFFICIAL WORK COMPLETION RECORD - FORM YT-Borewell</span>
        </div>
...
export default function YTCompletionBorewellReportPage() {
  return (
