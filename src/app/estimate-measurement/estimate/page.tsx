...
function EstimateEntryPage() {
  ...
  const handleSave = () => {
    ...
    startTransition(() => {
      ...
      toast({ title: isUpdate ? 'Estimate Updated' : 'Estimate Saved', description: 'Technical records synchronized with ledger.' });
    ...
    <div className="p-4 sm:p-8 space-y-8 bg-slate-50 min-h-screen pb-32 text-black text-left">
      ...
      <div className="bg-white border border-slate-200 p-8 rounded-[32px] shadow-sm ring-1 ring-slate-200/50">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tighter leading-none">ESTIMATE PORTAL</h1>
          ...
          <div className="flex items-center gap-4">
            <div className="space-y-1">
                <Label className="text-[9px] font-black uppercase text-slate-400 ml-1">Estimate Date</Label>
                <Input disabled={!isAllowed} type="date" value={formData.reportDate} onChange={(e) => updateField('reportDate', e.target.value)} className="h-10 w-44 rounded-xl font-bold bg-white text-black" />
            </div>
            <div className="space-y-1">
      ...
      <div className="flex justify-between items-center bg-blue-50/30 p-8 rounded-[32px] border border-blue-100">
          <div className="flex items-center gap-4">
              ...
              <div>
                  <h4 className="font-black text-[11px] text-blue-900 uppercase tracking-widest leading-none mb-1">Central Sync Mode</h4>
                  <p className="text-[9px] font-bold text-blue-400 uppercase">Records will be synchronized with the district TECHNICAL registry.</p>
              </div>
          </div>
          <Button onClick={handleSave} disabled={isPending || !isAllowed} className="h-16 px-16 rounded-full bg-[#1e3a8a] text-white font-black uppercase tracking-widest text-[11px] shadow-xl shadow-blue-900/30 gap-3 hover:bg-blue-900 transition-all hover:scale-[1.02] active:scale-95">
            {!isAllowed && <Lock className="size-4" />}
            {isPending ? <Loader2 className="size-5 animate-spin" /> : <Save className="size-5" />} 
            {isAllowed ? (id ? 'UPDATE TECHNICAL LEDGER' : 'SYNCHRONIZE TO LEDGER') : 'ACCESS RESTRICTED'}
          </Button>
      </div>
    </div>
  );
...
export default function EstimateEntryPageWrapper() {
    return <Suspense fallback={<div className="p-12 text-center animate-pulse">Initializing Technical Node...</div>}><EstimateEntryPage/></Suspense>;
}
