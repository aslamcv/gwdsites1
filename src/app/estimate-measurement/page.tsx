...
export default function EstimateMeasurementLedgerPage() {
  ...
  const allRecords = useMemo(() => {
    ...
    return reports
      .filter(r => {
        const isEMRecord = r.category === 'ESTIMATE_MEASUREMENT' || r.reportType;
        if (!isEMRecord) return false;
        const term = searchQuery.toLowerCase().trim();
        if (!term) return true;
        const searchableFields = [r.nameOfSite, r.fileNo, r.applicantName, r.lsgd].map(f => (f || '').toLowerCase());
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 animate-in fade-in duration-700 text-left">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-bold text-[10px] uppercase tracking-widest mt-2">Technical Estimates & Measurement Book (MB) Records</p>
        </div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-[300px] p-2 rounded-[24px] border-slate-200 shadow-2xl bg-white/95 backdrop-blur-xl">
            <DropdownMenuLabel className="px-4 py-3 text-[10px] font-black uppercase text-slate-400 tracking-widest">Select Process</DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem asChild className="rounded-xl cursor-pointer p-3 focus:bg-blue-50 group">
              <Link href="/estimate-measurement/estimate" className="flex items-center gap-3">
                <div className="p-2 bg-blue-100 rounded-lg group-focus:bg-blue-200"><Calculator className="size-4 text-blue-600" /></div>
                <span className="font-bold text-xs uppercase text-slate-700">New Estimate</span>
              </Link>
            </DropdownMenuItem>
      ...
      <Card className="border-none shadow-sm ring-1 ring-slate-200 rounded-[24px] overflow-hidden bg-white/80 backdrop-blur-md p-4">
        <CardContent className="p-0">
          <div className="relative">
            ...
            <Input placeholder="Search Site, File No or Contractor..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-12 pl-12 bg-slate-50 border-none rounded-xl text-sm font-medium focus:ring-primary/20" />
          </div>
        </CardContent>
      </Card>
            ...
            <Table>
              ...
              <TableBody>
                {isLoading || isUsersLoading ? (
                  ...
                  paginatedRecords.map((r) => {
                    ...
                    return (
                      <TableRow key={r.id} className="h-20 border-slate-50 hover:bg-slate-50/50 transition-colors group">
                        ...
                        <TableCell>
                           <button onClick={() => { setSelectedEmReport(r); setIsEmModalOpen(true); }} className={cn("h-7 px-3 text-[9px] font-black uppercase rounded-lg border-none shadow-sm flex items-center justify-center mx-auto transition-colors", r.reportType === 'ESTIMATE' ? "bg-blue-50 text-blue-700 hover:bg-blue-100" : "bg-emerald-50 text-emerald-700 hover:bg-emerald-100")}>
                              <FileSpreadsheet className="size-3 mr-1.5" /> View {r.reportType} Report
                           </button>
                        </TableCell>
                        <TableCell><Badge variant="secondary" className="text-[9px] font-black uppercase tracking-tighter bg-slate-100 text-slate-500">{r.reportType || 'RECORD'}</Badge></TableCell>
                  ...
                  <TableRow><TableCell colSpan={7} className="h-64 text-center opacity-20"><FilterX className="size-16 mx-auto mb-4" /><p className="font-black uppercase text-sm">No E/M records found</p></TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
      ...
      <AlertDialog open={!!reportToDelete} onOpenChange={(open) => !open && setReportToDelete(null)}>
        <AlertDialogContent className="rounded-3xl p-8">
            <AlertDialogHeader className="flex flex-col items-center text-center">
                ...
                <AlertDialogTitle className="text-2xl font-black text-slate-800 uppercase tracking-tight">Delete E/M Record?</AlertDialogTitle>
