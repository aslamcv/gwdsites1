...
interface PumpingTestStaffAssignmentProps {
  title: string;
  backUrl: string;
  nextUrl: string;
}
...
export function PumpingTestStaffAssignment({ title, backUrl, nextUrl }: PumpingTestStaffAssignmentProps) {
  ...
  const [otherStaff, setOtherStaff] = useState(Array(5).fill({ name: '', designation: '', district: '' }));
  ...
  const staffConfigs = [
    ...
    { 
      ...
      max: 1, 
      icon: UserCheck,
      designations: ['Hydrogeologist'] 
    },
    { 
      key: 'juniorHydrogeologist', 
      label: 'JUNIOR HYDROGEOLOGIST', 
      max: 1, 
      icon: Users,
      designations: ['Junior Hydrogeologist'] 
    },
    { 
      key: 'unitInCharge', 
      ...
      max: 2, 
      icon: UserCheck,
      designations: ['Master Driller', 'Senior Driller', 'Driller', 'Driller Mechanic', 'Drilling Assistant'] 
    },
    { 
      key: 'siteSupervisor', 
    ...
    { 
      ...
      max: 10, 
      icon: Eye,
      designations: ['Watcher', 'Skilled Worker', 'SLR', 'CLR', 'CLR (Employment)'] 
    },
    { 
      key: 'slr', 
      label: 'SLR', 
  ...
  const updateOtherStaff = (index: number, field: 'name' | 'designation' | 'district', value: string) => {
    if (!isAllowed) return;
    const updated = [...otherStaff];
    updated[index] = { ...updated[index], [field]: value };
    setOtherStaff(updated);
  };
  ...
  const handleContinue = () => {
    ...
    startTransition(() => {
      ...
      const reportData = {
        ...
        staffAssignment,
        otherDistrictStaff: otherStaff.filter(s => s.name),
        uploadedBy: user.uid,
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-2">
          ...
          <Select onValueChange={setConveyanceMode} value={conveyanceMode} disabled={!isAllowed}>
            <SelectTrigger className="h-11 border-primary/20 bg-white rounded-xl shadow-sm">
                    ...
                    {filteredEmployees.length > 0 ? (
                      filteredEmployees.map((emp) => {
                        ...
                        return (
                          ...
                          >
                            <Checkbox 
                              checked={isAssignedSelected} 
                              disabled={disabled}
                              onCheckedChange={() => handleToggle(config.key, emp.name, config.max)}
                              className="size-4 rounded-md border-slate-300 data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                            />
                      ...
                      <div className="flex flex-col items-center justify-center py-8 text-center opacity-40">
                        <Users className="size-4 w-4 mb-2" />
                        <p className="text-[9px] font-bold uppercase text-balance">No matching staff</p>
                      </div>
                    )}
                  </div>
                </ScrollArea>
      ...
      <Collapsible disabled={!isAllowed}>
        <Card className="border-none shadow-sm bg-slate-50 ring-1 ring-slate-200 rounded-2xl overflow-hidden">
          <CollapsibleTrigger asChild>
            <CardHeader className="bg-white py-3 px-4 border-b border-slate-200 cursor-pointer hover:bg-slate-50 transition-colors">
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-2">
                  <UserPlus className="h-5 w-5 text-primary" />
                  <CardTitle className="text-[10px] font-black uppercase tracking-widest text-slate-700">OTHER DISTRICT STAFFS (Limited to 5)</CardTitle>
                </div>
          ...
          <CollapsibleContent>
            <CardContent className="p-4 space-y-3">
              <div className="grid grid-cols-12 gap-4 mb-1 px-1">
                <div className="col-span-4 text-[9px] font-black uppercase text-muted-foreground tracking-widest">Name</div>
                <div className="col-span-4 text-[9px] font-black uppercase text-muted-foreground tracking-widest">Designation</div>
                <div className="col-span-4 text-[9px] font-black uppercase text-muted-foreground tracking-widest">District</div>
              </div>
                ...
                <div key={idx} className="grid grid-cols-12 gap-3 items-center">
                  <div className="col-span-4">
                    <Input 
                      ...
                      className="h-10 text-xs border-slate-200 bg-white rounded-lg shadow-sm" 
                      value={staff.name}
                      onChange={(e) => updateOtherStaff(idx, 'name', e.target.value)}
                      disabled={!isAllowed}
                    />
                  </div>
                  <div className="col-span-4">
                    <Input 
                      ...
                      onChange={(e) => updateOtherStaff(idx, 'designation', e.target.value)}
                      disabled={!isAllowed}
                    />
                  </div>
                  <div className="col-span-4">
                    <Input 
                      ...
                      onChange={(e) => updateOtherStaff(idx, 'district', e.target.value)}
                      disabled={!isAllowed}
                    />
                  </div>
                </div>
